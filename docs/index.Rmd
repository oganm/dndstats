...
corFrame %>% ggplot(aes(x = oganm,y = FiveThirtyEight,text = paste(class, race))) +
    geom_point() + 
    ggtitle("Class-race combination %s at\n FiveThirtyEight vs oganm's data") ->p
...
[data in DnDBeyond](https://twitter.com/BadEyeAdam/status/969435420676231169).




```{r featBar}
featedChars = singleClassed %>% filter(!is.na(feats)) %>%
...
featFrame %>% 
    ...
    scale_fill_manual(values = c('#7DD4A6','#C15BC5','#D65242','#415455','#D2A75C','#8FD25B',
                                 '#D15B86','#A5B5BE','#727EC6','#567441','#754334','#5E3A60'))->p
...
going into that rabbit hole... How long before the a game a player may want their character 
...
This is of course my humble opinion (in fact `` characters in my corpus are named "Test").

When I started collecting data I was hoping to release an anonymzed version
